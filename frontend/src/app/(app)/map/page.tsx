...
export default function MapPage() {
  ...
  // Leaflet is loaded dynamically (it touches window), so keep the namespace here.
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      for (const d of drivers) {
        ...
        // Draw the current trip's pickup → dropoff route.
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-[0_2px_10px_-2px_rgba(30,34,43,0.06)]">
        <div ref={containerRef} className="h-[70vh] w-full" style={{ zIndex: 0 }} />
      ...
      {updatedAt && (
        <p className="flex items-center gap-1.5 text-xs text-slate-400">
          <RefreshCw className="h-3 w-3" /> {c("updated")}: {updatedAt.toLocaleTimeString(locale)}
        </p>
      )}
    </div>
  );
