...
Uber's 0,0 coords are nulled; malformed rows are per-row try/catch so one bad row never fails a batch. After each batch it runs `expirePending()` (**no accept-window timeout** — Pending → Rejected only when the driver is now OFFLINE, i.e. they left without taking it, plus a 2h hard-cap backstop for a dead/unlinked session) and `finalizeStale()`. A pending offer is otherwise resolved by an event: a newer offer supersedes it, or the driver engages and it's accepted.
