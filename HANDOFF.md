...
- **2026-09-09 — An IDLE driver's un-taken offer now expires (was stuck "Open" forever).** The pure event model rejected a pending offer only on the observed engaged→idle EDGE (`rejectPendingFor`) — so an offer that arrived while the driver was ALREADY idle, or whose engaged→idle edge a coarse poll skipped, sat "Open" indefinitely (seen live: a 0.9 km offer pending for an hour while its driver was online-idle doing nothing). `expirePending` now resolves a pending offer by the driver's current availability, robustly (independent of catching the exact edge): **HOLD while ENGAGED** (en route / on a trip — a possible back-to-back — bounded only by the 2 h cap); **REJECT when IDLE-online and the accept window (+30 s grace) has elapsed** (the driver was free and passed on it); **REJECT when OFFLINE**; **REJECT past the 2 h cap**. A late-detected take within the 3-min grace still overturns it. This restores the idle-timeout that f3f3b3f had dropped, but ONLY for idle/offline drivers — an engaged driver is never timed out, so the back-to-back flicker that motivated the event model can't return. `displayStatus` stays stored-status-only (no display-layer inference), and the engaged→idle edge reject stays as the instant fast-path. Runs every poll (the ingestor sweep, throttled 15 s) and every minute (`offers:expire-pending`), so a stuck idle offer clears within ~1 min. Tests: `test_an_idle_drivers_pending_offer_expires_once_the_window_passes`, `test_an_offline_drivers_pending_offer_is_rejected` (replacing `test_a_pending_offer_is_rejected_only_once_the_driver_goes_offline`). Backend-only (CI). Files: `OfferLifecycle.php` (`expirePending`), `OfferAcceptanceTest.php`.
- **2026-09-08 — The offer PUSH now Latinizes the address (was showing rider-localized foreign text).** Uber localizes an offer's address TEXT to the RIDER's app language, so a Chinese rider produced a push body like "德国伍珀塔尔邮政编码: 42103". The app's detail card was already correct (the API resource Latinized it), but the **push** read the raw `dropoff_address` through `AddressFormatter::tidy` — which only *strips* non-Latin comma-segments and falls back to the original when a segment is entirely non-Latin — so the foreign string reached the lock screen. Root cause was duplication: the "non-Latin → authoritative German `<PLZ> City`" rule lived privately inside `DispatchOfferResource`, and the notifier never applied it. Extracted the single source of truth to **`App\Domain\Dispatch\AddressLatinizer::toLatin(display, raw)`**; the resource now delegates to it and `DispatchNotifier` uses it for the push **body** and the `data.pickup`/`data.dropoff` (single + multi-stop), preferring the resolved `*_display` and replacing any non-Latin value with `<PLZ> City` (PLZ↔city is 1:1 in Germany), or blanking it when there's no postcode. **Script-agnostic** (`AddressNormalizer::hasNonLatinLetters` = "any non-Latin letter"), so Cyrillic/CJK/Devanagari/Arabic are all covered by the same check — verified. A Latin foreign spelling (French "Cologne" for "Köln") is readable and passes through unchanged. Test: `DispatchNotificationTest::test_a_rider_localized_non_latin_address_is_latinized_in_the_push`. Backend-only (push text is server-generated → reaches devices on the next deploy, no app update). Files: `AddressLatinizer.php` (new), `DispatchNotifier.php`, `DispatchOfferResource.php`.
- **2026-09-08 — Offer-time geocoding recovers the base city from a Stadtteil name.** A drop-off named only by its district and no PLZ ("Heckinghauser Str. 102, **Wuppertal-Heckinghausen**") resolved to nothing usable at offer time: the district isn't a listed town, so `townOf()` returned null → no city, no PLZ → the free-text `approx` tier → the distance was withheld and the **push carried no km/€-per-km in the 5-second window** (only Uber's post-accept waypoints later filled it, `geo_source='uber'`, too late for the notification). `TripGeocoder::knownTown()` now falls back from a `City-Stadtteil` name to its base city (validated against the postal-code table both ways), so Tier 1b (street + city) fires and `completePostcode` then infers the missing PLZ from that result. It only falls back when the FULL name is unknown, so a genuinely hyphenated town (Castrop-Rauxel, Kamp-Lintfort) is never truncated, and a street word is still never taken for a town. Stays on self-hosted Nominatim (no new geocoder). The other two sample offers (`Max-Horkheimer-Straße 42119 …` glued PLZ, and a clean street+PLZ pair) already parse correctly — their offer-time miss was the 2.5s cold-cache pre-notify deadline, not addressing. Test: `OfferTripTest::test_town_of_recovers_the_base_city_from_a_stadtteil_district_name`. Backend-only (CI). Files: `TripGeocoder.php`, `OfferTripTest.php`.
- **2026-09-08 — A driver returning to idle rejects an un-taken pending offer.** Closes the last gap in the event-driven model: when a driver finishes a trip and goes back to **idle-ONLINE** (engaged → idle), the trip's offer completes as before AND any still-**pending** offer of theirs — a back-to-back offer that arrived during the trip but was never taken — is now **rejected** (they're available again and never engaged on it → passed on it). New `OfferLifecycle::rejectPendingFor(tenantId, driverUuid)`, called from `DriverStatusIngestor`'s engaged→idle branch **only on a genuine idle-ONLINE return** (an OFFLINE blip is left to the offline sweep). A coarse poll that briefly reports idle between two back-to-back trips is safe — the 3-min late-accept grace overturns the rejection when the driver actually engages on it a poll later. This is exactly the "why is an idle driver's offer stuck pending for 30 min" case. The ingest response gains a `rejected` count. Test: `test_returning_to_idle_completes_the_trip_and_rejects_an_untaken_pending_offer`. Files: `OfferLifecycle.php`, `DriverStatusIngestor.php`, `OfferAcceptanceTest.php`.
- **2026-09-08 — Offer state is purely event-driven: NO accept-window timeout.** The accept window no longer rejects an offer at all — not on display, not in the sweep. A **pending** offer stays pending until an EVENT resolves it: (a) a **newer offer** arrives → the older pending one is superseded → rejected (`supersedePendingFor`, one pending per driver, idle or engaged); (b) the **driver engages** (EN_ROUTE/ON_TRIP) → the pending offer is accepted (`DriverStatusIngestor`); (c) the **driver goes OFFLINE** → they left without taking it → rejected (`expirePending`, now `whereHas('driver', offline())` + a 2h hard-cap backstop for a dead/unlinked session). `DispatchOffer::displayStatus()` is now just `return $this->status ?? Pending` — it no longer infers "rejected" from an elapsed window while idle, which was flipping a still-live offer to rejected then back to accepted (the "how did a rejected offer become completed / why is this stuck pending" reports). An offer is **never** completed/accepted unless the driver's own status shows they took it. Late-accept overturn (a rejection reversed within `LATE_ACCEPT_GRACE_MINUTES` = 3) is unchanged. Tests: `test_late_acceptance_overturns_a_recent_rejection`, `test_a_past_window_pending_offer_still_reads_pending`, `test_a_pending_offer_is_rejected_only_once_the_driver_goes_offline` (replacing the three window-timeout tests). Backend-only (CI). Files: `OfferLifecycle.php` (`expirePending`), `Models/DispatchOffer.php` (`displayStatus`), `OfferAcceptanceTest.php`.
- **2026-09-07 — "Open in Maps" now pins by EXACT coordinate, not the address text.** The driver-app maps deep link passed only the address TEXT (`origin=<pickup text>`), so Google re-geocoded it — a house-number-less or degraded ("PLZ City" fallback) address landed on the wrong spot, EVEN when we held Uber's exact post-accept coordinates. Now `DispatchOfferResource` exposes `pickup_lat/lng`, `dropoff_lat/lng`, `geo_source` (and each `stops[]` carries `lat/lng`), the push `data` carries them too, and `openRouteInMaps` pins by `lat,lng` when the coords are Uber's (`geo_source === 'uber'`, i.e. exact) — falling back to the text (Google's geocoder, better than ours) for imprecise pending offers. Fixes "the pin/address is wrong in maps". Backend via CI; driver-app via OTA (`eas update` — JS only, maps.ts/api.ts/callers). Files: `DispatchOfferResource.php`, `DispatchNotifier.php`, `driver-app/src/lib/{maps,api}.ts`, `driver-app/app/{(tabs)/index,offer/[id],_layout}.tsx`.
- **2026-09-07 — Second ultra-review: 7 confirmed findings fixed.** (a) **Daemon secondary-channel churn** (regression from the cookieFp fix): only the primary now advances `cookieFp`; `reconcile()` ADOPTS the primary's rotation into the secondary (matches the primary's live fp = self-rotation) instead of tearing it down — so ramenphx no longer restarts + resets seq every rotation. (b) **Waypoint OSRM-down distance** is flagged `estimated` (→ `~`), not a fake `exact`. (c) **`PostalCodes::nearest()`** memoized in-process (`$allCentroids`) — the fleet-map poll read the 8k-row table ~120×/request. (d) **Offline-blip vs stale-started**: idle→engaged only treats a surviving STARTED offer as a blip-resume when the driver was actually OFFLINE before (`wasOffline` passed into `applyTransition`); a plain idle→trip start still finalizes a stale STARTED offer. (e) **Offers CSV export** streams via `lazyByIdDesc()` (keyset, stable) not `lazy()` (offset — dup/skip rows during live ingest). (f) **CSV formula injection** neutralized via `App\Support\Csv::cell()` on rider/address/company cells in the offers + billing + collector exporters. (g) **Tier 1b reachable**: `$city` falls back to `townOf()` when there's no PLZ, so a street-only borrowed-town pickup resolves precisely instead of the free-text `approx` tier. Tests added for (d), (g). The one un-fixed finding was a false positive.
- **2026-09-07 — Rejected fix: don't supersede an ENGAGED driver's older offer.** A driver on a trip was seeing an older pending offer flash **Rejected** the instant a newer offer arrived, then flip to accepted when they actually did it (Uber batches back-to-back trips — confirmed in the field). `OfferLifecycle::supersedePendingFor` now loads the driver and **returns 0 while engaged** (`engagementStatus() >= 1`), holding older offers pending for a possible back-to-back trip; it still supersedes when the driver is **idle** (a new offer then means the old was passed). `expirePending`/the next engagement resolve held offers once idle. Test `test_a_new_offer_does_not_supersede_an_engaged_drivers_prior_offer` updated to the new behavior.
- **2026-09-07 — Street-level approximate distance now gated by a min trip length.** The street-level relaxation gave a distance for a short same-street hop ("Hoheleye → Hoheleye 3") whose ~mid-street error dominates (65 m ↔ 877 m between runs). `TripGeocoder` now trusts a street-level (non-both-exact) distance only when the route is ≥ `MIN_APPROX_DISTANCE_M` (1.5 km); shorter street-level trips stay blank until Uber's waypoints. Both-exact trips are unaffected. Test renamed `test_short_street_level_trip_leaves_the_distance_blank_no_guess`.
- **2026-09-07 — Airport/POI pickups now yield an approximate distance.** Uber sends a bare `Dusseldorf Airport` pickup (no street/PLZ) → it resolved via free-text as low-confidence `'approx'` and the distance was withheld, even though an airport is a single unambiguous point. `TripGeocoder::isAirport()` now tags a resolved airport/aerodrome hit `'street'` (trusted for an approximate `~` distance) instead of `'approx'`. Locked by `OfferTripTest::test_airport_pickup_resolves_to_an_approximate_distance`.
- **2026-09-07 — Street-level offers now get an APPROXIMATE distance (no-guess relaxed, scoped).** Uber often sends a drop-off as a known street + PLZ + city with NO house number → `'street'` confidence, which the no-guess policy previously left blank (no distance/€-per-km at the 5-second decision window). Now `TripGeocoder::enrich` computes a distance when BOTH ends are `'street'`-or-better (`preciseEnoughForDistance`): a street-level point sits mid-street in the RIGHT town, so the route is off by at most the street's length — small and self-correcting. `AREA`/`POSTAL`/`APPROX` ends (town centroid / ambiguous no-postcode hit — the source of the old 55 km / 877 m-vs-65 m garbage) stay withheld. The push flags it approximate: `buildMetrics` prefixes `~` whenever `geo_confidence !== 'exact'`; the accept's Uber waypoints upgrade it to exact (`~` drops). Locked by `OfferTripTest::test_street_level_drop_without_a_house_number_gets_an_approximate_distance`.
- **2026-09-07 — Multi-stop notification reformatted to the requested layout.** The second (multi-stop) push now leads with a **localized title** — `Zwischenstopp erkannt` / `Multi-stop detected` / `تم اكتشاف نقاط متعددة` (by `driver.locale`; this one push is intentionally worded, unlike the word-free single-offer push) — and a body of `🔵 x{N} · {km} km · €{rate}/km` followed by **every stop on its own bulleted line** (pickup, then each drop-off with its `+km`). Replaces the old `🟦×N` title + `→`-prefixed body (`DispatchNotifier::notifyMultiStop` + `buildStopBullets`). Note: an OS push can carry only an emoji (🔵), not the blue-circle/white-square stop icon — the exact custom icon needs an in-app banner if wanted. Backend-only (deploys via CI, no OTA).
- **2026-09-06 — Ultra-review fixes (9 confirmed findings from a 72-agent correctness/perf/security sweep).**
  - **Daemon churn (biggest offer-loss cause):** the supervisor tore down a healthy RAMEN stream every ~60s because `absorbCookies` rotated + persisted the session cookies but never updated the stream's own `cookieFp`, so `reconcile()` mistook the daemon's own rolling refresh for an external re-link (teardown reset `seq` to 0 → blind window → dropped offers). `absorbCookies` now recomputes `cookieFp` after a rotation (and the secondary channel also absorbs into its jar so it doesn't go stale); a genuine re-link still restarts (its jar comes from a fresh capture, not a Set-Cookie).
  - **EN_ROUTE offline blip no longer cancels an accepted offer.** The offline grace protected only STARTED trips; an ACCEPTED (EN_ROUTE) offer was canceled immediately on an OFFLINE edge and, being terminal + `accepted_at`-stamped, was never recovered — losing the real trip + fare. Now an ACCEPTED offer cancels only on a genuine idle-ONLINE end; an OFFLINE blip leaves it for `finalizeStale` (`ACCEPTED_STALE_MINUTES`) and the idle→engaged branch starts it if the driver reconnects straight to ON_TRIP.
  - **Invoice number** is now persisted INSIDE the lock transaction (was written after commit → `UNIQUE(invoice_no)` race → 500 on concurrent activations, worst at year rollover).
  - **Suspended-manager guard** `user.active` (`EnsureUserTenantActive`) added to the dashboard group — hard-blocks only **banned/disabled** companies (admin abuse actions); `expired`/`inactive` deliberately keep dashboard access to view + reactivate (only Uber ingest is gated). `user.account` added to the admin group so a driver token hits a clean 403 instead of a 500 (`hasRole()` on a Driver).
  - **CSRF:** the two network-log clear calls now send `withCsrf:true` (were always 419). **FCM OAuth** token mint got a 3s/4s timeout (could stall the 5s push path ~30s). **Password reset** now revokes existing Sanctum tokens (driver + manager) so a stolen token dies on reset. **Offers CSV export** streams via `->lazy()` instead of `->get()` (no OOM on a mature fleet).
- **2026-09-06 — Daemon: fast stream reopen so a reconnect gap stops dropping offers.** The RAMEN stream drops constantly (`stream error: terminated` — a residential-proxy connection reset / normal long-poll cycle). `run()` treated every drop as a failure and waited the full `reconnectMinDelay` (2s) before reopening — a 2s blind window per cycle in which an offer Uber dispatched was never received (root cause of "driver got the Uber offer, Reidey never saw it"; confirmed: a linked driver on an actively-capturing tenant whose specific offer was absent from the Network feed). Fix: distinguish a stream that **actually opened then dropped** (benign cycle → reopen after `streamCycleDelay`≈250ms, resuming from `this.seq`) from a failure **before** opening (handshake/HTTP/404 → exponential backoff), plus a `dropStorm()` guard (≥6 sub-1.5s opens in a row → back off, so a hard-down proxy never hammers Uber). Shrinks the blind window ~10×. Does NOT change the observe-don't-control model. Complementary ops levers: a stickier residential proxy (fewer resets) and keeping a `vsdispatch.uber.com` tab open (the extension tees the live stream on the manager's real IP as a backup). Files: `dispatch-daemon/src/{stream,config}.js`.
- **2026-09-06 — Fix: invoice-template save 500 (`issuer_address cannot be null`).** The builder lets the admin blank any text line so it drops from the PDF, and `InvoiceTemplateController` persists an emptied field as `NULL` — but five `invoice_settings` columns were created `NOT NULL` (`issuer_name`, `issuer_address`, `invoice_title`, `footer_thanks`, `footer_terms`), so blanking any of them threw `SQLSTATE[23000] 1048`. New migration `make_invoice_settings_text_fields_nullable` makes them nullable via `->change()` (native, no dbal on Laravel 13). App code already treated them optional — this just aligns the schema.
- **2026-09-06 — Extension: opt-in "sign out my other Uber sessions" on Connect.** After a Connect, if the manager has explicitly opted in (a consent checkbox in the extension popup, stored `evictOtherSessions`), the extension opens Uber's own `account.uber.com/devices` in a background tab and `content.js` clicks Uber's real **"sign out all OTHER devices"** control — so Uber runs its Arkose bot-defense **transparently** (no forged tokens, no raw `deviceSignout` fetch). The click matcher requires a sign-out verb **AND** a scope word (`all`/`other`/`devices`/`alle`/`geräte`/…), so it can never hit a plain single-session logout; the CURRENT session (this browser + the cookies the daemon replays) is preserved by design. Armed via a fresh (<2 min) storage flag consumed once, so a normal later visit to account.uber.com signs out nobody. Popup flow triggers it directly (`evictSessions` msg); the dashboard Connect flow auto-arms it in the background when the pref is set. **Open risk (unchanged from the plan): unverified whether the daemon's server-side session — same cookies, different IP — counts as "current" and survives, or gets evicted; after enabling on prod, watch that offers keep streaming (the background poll's `reportBrokenSession` will flag a 401/403), and re-Connect if it dies.** Extension `1.15.4 → 1.16.0`. Files: `extension/{background,content,popup}.js`, `popup.html`, `manifest.json`.
- **2026-09-06 — Non-Latin address follow-ups: itinerary Latinized + a backfill command.** `DispatchOfferResource` now Latinizes each `stops[]` address too (not just the endpoints), so the detail-view / driver-app itinerary matches the clean German the list already showed (different surfaces read different fields — the list uses `dropoff_address`, the detail renders `stops[]`). New **`offers:backfill-addresses`** command re-reverses the coordinates already stored on OLD offers — those whose reverse failed transiently at sync time and are locked (`geo_source='uber'`) on a partial/non-Latin label — into a full German address and syncs the `stops[]` endpoints; run once on prod (upgraded 29). Ceiling is the source data: a drop-off in an industrial area with no OSM house number resolves to street + PLZ + city (Uber never sent a number either), and a point with no PLZ shows blank — never foreign text. The reverse always requests `accept-language=de`, and `AddressNormalizer::hasNonLatinLetters` (Unicode `\P{Latin}`) covers **every** script (Cyrillic/Arabic/Devanagari/CJK/…), so this is language-agnostic, not Japanese-specific.
- **2026-09-06 — Invoice template builder: editable headings + custom footer blocks + up/down reorder.** Extended the invoice template editor so every fixed German heading is overridable and the footer fine-print is now an editable, reorderable set of column blocks. **One migration** (`add_labels_and_footer_blocks_to_invoice_settings`) adds two nullable JSON columns to `invoice_settings`: `labels` (a flat map of heading overrides) and `footer_blocks` (`[{heading, lines:[{label,value}]}]`). `InvoiceSettings` casts both to `array`, adds them to `$fillable`, and gains `label(key)` (override-or-German-default, keys held in a private `LABEL_DEFAULTS` map) and `footerBlocks()` (returns saved blocks; else **derives the legacy 3 columns** — issuer / Kontakt / Steuer & Bank — from the flat issuer/bank fields, stripping empty lines/blocks, so an install with `footer_blocks=null` renders **identically** to before). The Blade (`invoices/invoice.blade.php`) replaced every hard-coded heading (`bill_to`, `invoice_date`, `period`, `activation_code`, `customer_no`, `invoice_no`, `description`, `qty`, `unit_price`, `amount`, `subtotal`, `total`, `paid`, `paid_at`, `payment_method`, `sold_by`, `invoice_no`) with `{{ $settings->label('…') }}` and loops the footer over `footerBlocks()` into equal-width `<td>` columns (all output escaped; still dompdf-safe table-only, DejaVu Sans). `InvoiceTemplateController@update` validates `labels`/`labels.*` and the nested `footer_blocks.*` and adds both to `present()`. Editor (`admin/invoice-template/page.tsx`): a **Labels / Headings** grid (one input per key, German default as placeholder) and a **Footer blocks** section (heading + label/value line editors, add/remove block & line, **up/down reorder** of blocks and lines via lucide `ChevronUp/Down`; when the server sent `footer_blocks=null` it seeds the 3 derived blocks client-side, mirroring the model). Types in `lib/api/invoice-template.ts` (`labels`, `footer_blocks` + `FooterBlock`/`FooterLine`); new `screens.invoiceTemplate.*` keys in **de/en/ar**. Debounced save-then-preview already re-renders on any change, so labels/blocks live-preview. **Free x/y drag positioning was NOT done** — dompdf is table-only; scope is editable labels, custom blocks, and up/down reorder only. Tests: `labels` override swaps a heading (un-overridden keys still fall back to German), a `footer_blocks` array renders custom columns (empty-value lines stripped), and `footer_blocks=null` still derives the 3 legacy columns. `pint --test` + `php artisan test --filter=Invoice` (17) green; `npm run build` clean.
- **2026-09-06 — Dispatch address display is never non-Latin (rider-localized text).** Uber localizes an offer's `pickup`/`dropoffAddress` TEXT to the RIDER's app language, so a Japanese rider produced e.g. dropoff `"ドイツ 〒42781 ハーン グルイテン"` — an unreadable string shown to the driver/dashboard in place of a German address. New `TripGeocoder::latinDisplay(?current, ?raw)` guarantees a Latin value: a Latin display (the normal German address) passes through untouched, a non-Latin one is replaced with the authoritative `"<PLZ> City"` from the always-Latin 5-digit postcode inside it (`PostalCodes::city`, PLZ↔city is 1:1 in Germany — correct, not a guess), and a non-Latin value with no usable PLZ becomes blank (better than foreign text). Applied in `enrich()` after `completePostcode` (offer-time, TEXT only → PLZ+city) and in `applyFromWaypoints()` after `labelForPoint` (post-accept, German reverse preferred; PLZ+city only if the reverse failed and the value is still non-Latin, so a transient Nominatim outage never leaves Japanese stuck even with `geo_source='uber'`). Belt-and-suspenders in `DispatchOfferResource` (new `latinAddress()`): the exposed `pickup_address`/`dropoff_address` apply the same PLZ+city fallback, covering an offer viewed before geocoding whose `*_display` is still null. The stored raw `pickup_address`/`dropoff_address` columns are untouched — only `*_display` and the exposed resource value. Test in `OfferTripTest` (reflection, network-free): a Japanese address → `"42781 Haan"`, a normal German address passes unchanged, non-Latin without a PLZ → null. `pint --test` + `php artisan test` green (280).
- **2026-09-06 — Invoice template: empty fields vanish, everything optional.** Any invoice field the admin leaves blank is now dropped from the rendered PDF instead of showing a placeholder or an orphan heading. `InvoiceTemplateController@update` made every text field **nullable** (only the structural `number_prefix`/`currency`/`vat_rate`/`kleinunternehmer` stay required) and stores an emptied field as `NULL` (not `''`). The Blade (`invoices/invoice.blade.php`) now `@if`-guards the brand name, `footer_thanks`/`footer_terms`, and each footer column **plus its heading** (Kontakt / Steuer & Bank / issuer) — a column renders only when it has content — and finally shows **`bank_name`** in the Steuer & Bank block (it was collected but never printed). The `InvoiceSettings::current()` seed no longer ships **fake tax-ID/IBAN/BIC/bank-name** defaults (a placeholder VAT/IBAN is worse than none). Editor (`admin/invoice-template/page.tsx`): inputs are null-safe and carry example **placeholders**. Existing installs keep their saved values — clear the demo tax/bank in the editor and Save to drop them. (A drag/reorder/custom-block "builder" is a separate, larger feature — dompdf is table-only, so free x/y positioning would need a different PDF engine.)
- **2026-09-05 — Proxy subscription renewals (history + running total).** A residential proxy can now be **renewed on the same credentials** (the invoice is paid again for another span): `Admin\ProxyController@renew` (`POST admin/proxies/{proxy}/renew`) records one paid period (amount + start/end) in the new **`proxy_renewals`** table. The proxy's spend accumulates (`totalPaid()` = initial price + Σ renewal amounts) and its **real expiry** becomes the furthest end across the base period and every renewal (`effectiveEndsAt()`, surfaced as `ends_at`; the base row's `expires_at` stays the first period, so the edit form still edits it). Proxies also gained a base **`starts_at`**. Frontend (`admin/proxies/page.tsx`): a **Renew** button + modal (amount · start · end · note) showing the running **total paid**, current expiry and the full **renewal history** (initial purchase + each renewal), a start-date field on the edit form, and a total-paid line in the Cost cell; `renewProxy` in `lib/api/admin.ts`; de/en/ar i18n.
- **2026-09-05 — Admin driver directory + overview fleet stat.** New super-admin page `/admin/drivers` (`Admin\DriverDirectoryController`, `GET admin/drivers`) lists the whole **active fleet across every company** — each driver's live status (online + engagement idle/EN_ROUTE/ON_TRIP), the offer they're **currently on** (accepted/started, resolved in one query — no N+1), contact details, company and Uber rating/trips — with search (name/phone/email/company), a **company** filter and a **live-status** filter (all / online / available / en-route / on-trip / offline, via a new `Driver::scopeOffline()` + `online_status` LIKE buckets), pagination, and a top **online / online-rate / total** stat row (computed over `Driver::activeFleet()`, respecting the company filter, orphans excluded). The admin **overview** dropped its per-driver online list for a compact **drivers stat card** (online · rate · total) linking to the new page: `OverviewController` now returns `stats.drivers_total` + `stats.drivers_online` (activeFleet) and no longer emits `online_drivers`. Frontend: `admin/drivers/page.tsx`, `getAdminDrivers` in `lib/api/admin.ts`, a `nav.driverDirectory` entry, and full de/en/ar i18n (`adminDrivers` namespace + `adminDashboard` card keys).
- **2026-09-05 — Multi-stop push mark is a blue square (🟦×N), not a circle.** The multi-stop notification's language-neutral lead glyph switched from 🔵 to **🟦** in `DispatchNotifier::notifyMultiStop` (title + body), so the push mirrors the app/dashboard blue *square* stop marker instead of a circle. Push text is server-generated, so this reaches devices on the next backend deploy — no app update needed.
- **2026-09-05 — One active trip per driver + live-map drop-off pin fix.** Three linked fixes to how in-flight trips read. (1) **Phantom "active" trips** (a driver showing on two trips at once, active-trip count > online drivers, an offline driver still "on a trip"): a driver holds exactly ONE active trip, so `OfferLifecycle::supersedeActiveFor()` finalizes any OTHER still-open offer of theirs (complete a STARTED one, cancel an ACCEPTED one) — called from `DriverStatusIngestor` at every accept (idle→engaged, back-to-back) and on idle (keeping the flicker-guarded latest). This closes the stragglers the 60-second flicker guard or a rapid trip-to-trip jump left STARTED, instead of waiting for the 100-min stale sweep. **Offline vs. a mid-trip connection blip:** a STARTED trip is completed on a genuine idle-ONLINE end (driver now available) but NOT the instant Uber reports OFFLINE — because a driver whose internet drops mid-trip is reported OFFLINE and would end the trip early. Drivers now carry `went_offline_at` (stamped when they go offline in `DriverStatusIngestor`, cleared on return), and `finalizeStale` completes an offline driver's STARTED trip only after `OFFLINE_GRACE_MINUTES` (5) of continuous offline — so a brief blip that returns to ON_TRIP keeps the live trip, while a real sign-off closes within ~5 min (the 100-min cap remains the final backstop). (2) **Fleet home active-trips list** (`Driver\FleetController::home`) now shows a LIVE trip only — the driver must still be online (`isOnline()`) and at most one trip per driver (newest) — so an offline driver's stale started offer never appears. (3) **Live-map drop-off marker**: once ON_TRIP Uber trims the pickup waypoint, so the lone remaining DROPOFF fell through the `wp[0]` pickup-fallback and rendered as a blue rider disc; it now stays a red drop-off pin (`live-map.tsx` — a waypoint typed DROPOFF is never the pickup). Multi-stop unaffected (intermediate stops stay red pins, pickup stays the blue rider). `php artisan test` green (44).
- **2026-09-05 — Offer state labels unified to the real state names (de/en/ar, everywhere).** The `rejected` and `canceled` states were shown under a jumble of wordings ("Not taken", "Declined", "Nicht angenommen", "غير مقبول", "ملغى بعد القبول") across `screens-ridy.ts`, `screens-admin.ts` (dashboard + admin) and the driver app's `i18n.ts` (`status.*`, `stat.declined`, `profile.declined`, `notAccepted`/`statNotTaken`). Now uniformly **Rejected / Abgelehnt / مرفوض** and **Cancelled / Storniert / ملغى**. The enum values (`OfferStatus`) were already `rejected`/`canceled` — this was display text only; German/Arabic were mostly already correct, English was the outlier ("Declined").
- **2026-09-05 — Two offer lifecycle-inference fixes (status flicker + supersede-while-engaged).** (1) **False "completed" from a status flicker (offer 5807).** Uber briefly reported `ON_TRIP` (~10 s) just before the driver's real `EN_ROUTE`; `DriverStatusIngestor` read the blip as a full pickup→dropoff and burned the fresh offer to `completed`, after which the genuine engagement a poll later could not reopen the terminal offer. New floor `MIN_TRIP_SECONDS = 60` + `tripLooksReal()` guard on **both** complete sites (back-to-back `2→1` and engaged→idle): a trip that returns to idle within 60 s of starting is a flicker → the offer stays `Started` for a later real idle (or the 100-min `finalizeStale` net) to finalize. `expirePending`/`finalizeStale` untouched. (2) **A new offer now supersedes an engaged driver's older pending offer too.** `OfferLifecycle::supersedePendingFor` dropped its engaged-skip guard (and the now-unused `Driver` lookup/import): Uber sends the next offer only once the previous is gone, so the older pending offer → **rejected** and the newest stays **pending**, whether idle or on a trip. Back-to-back is unaffected — a busy driver's single next offer is itself the newest (the kept one), never the row rejected here; the real back-to-back hold still lives in `expirePending` (engaged + under the 2-h hard cap). Tests: added a flicker regression + an engaged-supersede assertion; aged the instantaneous-trip lifecycle/back-to-back tests to a realistic 3-min trip. `php artisan test` green (39).
- **2026-09-04 — Subscription invoices: per-invoice PDF + editable template (from scratch).** There was NO invoice PDF or invoice email before — the "invoice" is a `SubscriptionPeriod` row (there is no `Invoice` model), and the only export was a CSV of all rows. Built the full pipeline: **dompdf** (`barryvdh/laravel-dompdf`), an `InvoiceSettings` singleton (`app/Domain/Billing/Models/InvoiceSettings.php` — issuer/bank/branding/accent/VAT/currency/texts, German defaults, `current()` firstOrCreate), a per-year sequential **`invoice_no`** on `subscription_periods` (`RE-YYYY-####`, atomic `lockForUpdate` in `InvoiceNumberGenerator`, assigned in `SubscriptionActivator` after create, `invoiceNumber()` falls back to `RE-{year}-{id}`), a **dompdf-safe Blade Rechnung** (`resources/views/invoices/invoice.blade.php` — 100% nested tables, DejaVu Sans for €/umlauts, no flex/grid, net/MwSt/gross split in cents, §19 Kleinunternehmer mode), an `InvoiceRenderer` (html/pdf/sample), and a super-admin **template editor**: `Admin/InvoiceTemplateController` (`GET/PUT invoice-template`, `POST invoice-template/image`, `GET invoice-template/preview` → HTML, `GET subscription-invoices/{invoice}/pdf` → PDF). Activation/renewal now **emails the invoice PDF** (`Mail/InvoiceMail`, best-effort, paid+amount only). Frontend: `admin/invoice-template` page (mirrors the email-template editor — logo+crop, accent, issuer/bank, VAT/Kleinunternehmer, texts, **live iframe preview** via debounced save-then-render), a per-row **PDF download** on the reports invoices table, i18n de/en/ar. **Deploy note:** the backend image must rebuild (composer changed) and `migrate --force` runs the two new migrations (seeds the settings row).
- **2026-09-04 — Online drivers on the admin overview.** `Admin/OverviewController` now returns `stats.drivers_online` + an `online_drivers` list (name · company · `engagement` 0 idle-online/1 EN_ROUTE/2 ON_TRIP, via `Driver::scopeOnline()` + `engagementStatus()`, across all tenants); the admin dashboard renders a live "Fahrer online" card (count badge + per-driver engagement chip). `frontend/src/app/(app)/admin/page.tsx`.
- **2026-09-04 — Geo: never guess a distance (blank unless both ends exact).** Uber sends addresses as strings only; a pickup with no house number geocodes to an arbitrary point on the street, which produced garbage distances/€-per-km for the SAME trip (877 m one run, 65 m another) and absurd rates (76 €/km). `TripGeocoder::enrich()` now fills `distance_m`+route ONLY when BOTH ends resolve to a precise house number (`'exact'`); a street/postal/centroid resolve leaves the distance **blank** (and thus no €/km) until a reliable source fills it — the driver accepts/finishes (Uber waypoints via `applyFromWaypoints`) or a later exact geocode. Confirmed via prod data: an accepted offer's 65 m WAS Uber's own waypoints (a real ultra-short canceled offer, not a bug); the 877 m was our address-geocode guess. Points/confidence still recorded; only the distance is withheld.
- **2026-09-04 — Custom notification sounds + unified stop marker + faster live poll.** The driver app bundles two alert sounds (`assets/sounds/normal.wav` / `multi.wav`) — a routine offer plays normal.wav on the "offers" channel, a multi-stop offer plays multi.wav on a MAX-importance "multistop" channel (urgent vibration); `FcmPushSender` sets `android_channel_id` by `stops_count`. **Sounds are a NATIVE change — ship via `eas build`, not OTA.** The blue "stop" marker (disc + rounded square) was unified to ~45 % symmetric across the app `StopMarker`, the dashboard `StopMarker`, and the trip-map drop-off marker. The daemon's adaptive `GetDriverLiveLocation` poll was tightened 4 s→3 s engaged / 10 s→6 s idle.
- **2026-09-04 — Error monitoring on 4 surfaces (Sentry) + a prod-outage lesson.** Sentry is now live on backend (`SENTRY_LARAVEL_DSN`), frontend (`NEXT_PUBLIC_SENTRY_DSN`), daemon (`DAEMON_SENTRY_DSN`) and the driver app (`EXPO_PUBLIC_SENTRY_DSN`, in `eas.json`), each its own project (EU region). The three server DSNs are GitHub secrets the CI writes into `.env`; the app DSN ships with the build. **Ops lesson (the hard way):** editing GitHub secrets to add the DSNs accidentally changed **`DB_PASSWORD`** — and because the CI **regenerates `.env` from secrets on every deploy**, the next deploy wrote a wrong DB password and the backend lost MySQL access ("Access denied for user 'ridy'"). MySQL is never recreated by the deploy, so its user password is the source of truth; recovery = `ALTER USER 'ridy'@'%' IDENTIFIED BY '<the .env DB_PASSWORD>'` via root (or revert the secret) + restart backend. Sentry caught it instantly. **Keep secret values saved externally — GitHub hides them, so a re-save can silently drift.**
- **2026-09-03 — Multi-stop notification + itinerary overhaul (blue stop marker, full-route map, pending-label fix).** A multi-stop push now leads with a blue stop mark (🔵×N) and lists **every** stop in the body — pickup, then each drop-off with its per-leg km — plus the km · €/km line (`DispatchNotifier::buildStopList`/`stopsPayload`); both the offer and multi-stop payloads carry `stops` as a compact JSON array so clients can route the maps app through all of them. `FcmPushSender` routes a `stops_count>=2` push to a new **`multistop`** Android channel (MAX importance + urgent vibration, created in `driver-app/src/lib/push.ts`) so extra drop-offs grab attention (a custom **sound file** still needs a native `eas build`). Both clients gained a blue **StopMarker** (circle + rounded-square) replacing the red drop-off pin: `driver-app/src/components/stop-marker.tsx` (in `RouteBlock`) and `frontend/src/components/ui/stop-marker.tsx` (offer-detail itinerary). **"Open in map" now routes through every stop** as Google Maps waypoints — including the notification "Open in map" action (`_layout.tsx` now parses `data.stops`; Apple Maps can't do multi-waypoint, so multi-stop always uses Google Maps). **Display fix:** a backend-`pending` offer (held for a busy driver's back-to-back trip) no longer reads as expired/declined from the client's local accept-window countdown — the status label derives from `offer.status` only, on both the app (`offer/[id].tsx`) and the dashboard (`offerBadgeStatus`). Diagnostics confirmed the lifecycle backend is correct (engaged offers held 5–49 min, rejected only at idle); the "not taken at 10s" was purely a display bug. Driver-app ships **OTA** (`eas update`). Still open: a custom multi-stop **sound** (needs an audio asset + native build), and the dynamic mid-trip **fare** source (investigating Uber's `earnerbreakdownsv2`/`supplierbreakdownv2` network logs — the offer stream carries a single fare only).
- **2026-09-03 — Notification placeholders, driver-app multi-stop surfaces, faster live-map poll.** Three fixes: (1) **Notifications rendered literal `{placeholders}`** — a template interpolates `{key}` from the caller's `params` (`NotificationPushText`), and two callers omitted a value: `proxy_expiring` ("{label} expires in {days} days") passed only `{label}` (`ScanNotifications`), and `session_needs_relink` ("{company}: the Uber session needs relinking") passed `[]` (`FleetSessionService`). Both now pass the value; audited every type — the rest were correct; `company_banned` has no live emitter (dormant). (2) **Driver-app multi-stop only showed on the offer detail** — the list rows (`RecentRow`, `OfferCard` which also backs the Offers tab) now show a compact "Mehrere Stopps · N" chip, and `openRouteInMaps` (shared by offer detail + home) appends the intermediate stops as Google Maps `&waypoints=` so the maps app opens the FULL ordered route, not pickup → final drop-off only. Ships **OTA** (`eas update`). Note: the push-action map in `app/_layout.tsx` still lacks stops (the push payload carries none). (3) **Live-map car looked laggy** — the daemon's adaptive `GetDriverLiveLocation` poll dropped from 4s→**3s** while a driver is engaged and 10s→**6s** idle (`dispatch-daemon` `config.js`, env-overridable); Uber refreshes a driver's location only every ~4-5s, so 3s catches each fix promptly without out-polling Uber. The 700ms client glide (`ac41232`) smooths the gaps. **Why not just WebSocket?** Reverb already delivers server→dashboard instantly; the limit is the extension/daemon→Uber hop — Uber gives no location stream, so we poll.
...
Started   → Completed
Rejected  → Accepted            (late-detected accept overturns a recent rejection, within the 3-min grace)
Completed → (terminal)
Canceled  → (terminal)
```
...
- `LATE_ACCEPT_GRACE_MINUTES = 3` — how recently a REJECTED offer must have been rejected to still overturn to Accepted (a late-detected take); past this it stays rejected.
...
- engaged → idle (genuine idle-ONLINE return): `complete()` the active offer if Started, else `cancel()` if only Accepted; AND `rejectPendingFor()` any still-pending offer the driver never took (available again → passed on it).

Uber's 0,0 coords are nulled; malformed rows are per-row try/catch so one bad row never fails a batch. After each batch it runs `expirePending()` and `finalizeStale()`. `expirePending` resolves a pending offer by the driver's availability: **HOLD while ENGAGED** (a possible back-to-back, until the 2h cap); **REJECT when IDLE-online and the accept window (+30s) has elapsed** (free and didn't take it); **REJECT when OFFLINE**; **REJECT past the 2h cap**. An engaged driver is never timed out (so a back-to-back never flickers rejected). A pending offer is also resolved by events: a newer offer supersedes it, or the driver engages and it's accepted.

**Safety nets (scheduled).** `offers:finalize-stale` (`App\Console\Commands\FinalizeStaleOffers`, every 5 min) force-completes STARTED offers older than 100 min and force-cancels ACCEPTED older than 20 min, across all tenants (covers idle daemons). `offers:expire-pending` (every minute) is the idle-tenant backstop that rejects a pending offer whose driver has since gone offline (or past the 2h cap).
