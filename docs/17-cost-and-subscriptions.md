...
**Netcup RS 4000 G12** (Nürnberg), with **Cloudflare** in front. Confirmed by the operator.
‏القرار: شركة واحدة، **حتى 10,000 سائق على بروكسي واحد**، على **Netcup RS 4000 G12** (نورنبرغ)،
و**Cloudflare** قدّامه. معتمد من المشغّل.
...
that load lands on the **server**, which is why a ~12-core / ~48–64 GB box is the right size.
‏البروكسي يحمل ستريم الشركة الوحيد فقط (ميغابايت قليلة/ساعة)، مستقل عن عدد السائقين. الـ10,000
سائق يكلّمون **سيرفرنا مباشرة** (لا عبر البروكسي) — وهذا الحمل يقع على **السيرفر**، ولهذا بوكس
بـ~12 نواة / ~48–64GB هو الحجم الصحيح.

### Netcup RS 4000 G12 · مواصفات (Nürnberg)

| Plan | vCPU (dedicated) | RAM | NVMe | Term · المدة | Est. monthly |
|---|---|---|---|---|---|
| **RS 4000 G12 ✅** | **~12 (dedicated)** | **~48–64 GB** | **~1 TB** | 12-month · 12 شهر | **confirm on order page** |

> **Root Server = dedicated cores** (steady perf), not a shared vServer. The `…-12m`
> variant is a **12-month minimum contract** (cheapest tier) — possible one-time setup fee.
> Confirm exact RAM/NVMe/price on the order page. Payment: PayPal / card / SEPA; a new
> account may face an identity check.
> ‏خط Root Server = أنوية مخصّصة (أداء ثابت). متغيّر `…-12m` **عقد 12 شهر** (الأرخص) وقد يكون
> فيه رسم إعداد. أكّد المواصفات/السعر من صفحة الطلب. الدفع: PayPal/بطاقة/SEPA؛ قد يُطلب تحقّق هوية.
...
|---|---|---|---|
| **Server** · السيرفر (Netcup **RS 4000 G12**) | Upgrade needed · لازم ترقية | **~€30/mo · شهري (confirm)** | ~12 dedicated vCPU / ~48–64 GB / ~1 TB NVMe — fits 10k drivers + geo · يتّسع لـ10 آلاف سائق + الجيو |
| **Cloudflare** (DNS/CDN/DDoS) | In front · قدّامه | **$0 (Free plan)** | DNS + caching + DDoS in front of the box · قدّام السيرفر |
...
| **Backups** · نسخ احتياطي (Netcup) | Recommended · مُوصى | **add-on / SCP snapshot · إضافة** | Backup Space add-on or manual SCP snapshots · مساحة نسخ أو سناب شوت يدوي |
...
- Monthly · شهري: **~€30** (RS 4000, confirm) + **$6.60** proxy + **$0** Cloudflare/email/push.
- ‏الإجمالي الفعلي: **~€30/شهر + $6.60 بروكسي + $25 لمرة وحدة** — أرخص بكثير من الكلاود بأنوية مخصّصة.
...
Row 1 is the **committed setup** (one company, 10k drivers, Netcup RS 4000, one proxy). The rest
assume each company brings its own Uber account (1 proxy each).
‏السطر الأول هو **الإعداد المعتمد** (شركة واحدة، 10 آلاف سائق، Netcup RS 4000، بروكسي واحد). الباقي بافتراض
...
|---|---|---|---|---|---|
| **1 company / 10k drivers ✅ · شركة / 10 آلاف سائق** | ~€30 (RS 4000) | $6.60 | $0 | $0 | **~€37 / mo** |
| **100 companies · شركة** | ~€30 (RS 4000, one box) | ~$660 | $20 | $0 | **~€650 / mo** |
| **500 companies · شركة** | ~€150 (split: app+DB+geo boxes) | ~$3,300 | $20 | $0 | **~€3,300 / mo** |
| **1000 companies · شركة** | ~€300 (full split + Redis + LB) | ~$6,600 | ~$100 | $0 | **~€7,000 / mo** |
...
- [ ] Order **Netcup RS 4000 G12** (Nürnberg), install Ubuntu 24.04 from SCP. · جهّز RS 4000.
- [ ] Put **Cloudflare** in front (DNS + Full-strict SSL + DDoS, free). · حط Cloudflare قدّامه.
- [ ] Set up backups (Netcup Backup Space add-on or scheduled SCP snapshot). · فعّل النسخ الاحتياطي.
